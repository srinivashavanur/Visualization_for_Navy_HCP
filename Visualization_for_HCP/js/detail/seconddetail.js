...
var insetWidth = insetRect.width - margin.left - margin.right, 
    insetHeight = height;

// Record last slider value
var d3slider = undefined;
var sliderValue = 0;
...
function draw_chart_detail_worker (workerid) {
  ...
  d3.json("http://www.cs.odu.edu/~hdo/InfoVis/navy/dataworkerid.php?workerid=" + workerid, function(err, data) {
    ...
    var inset = d3.select("#inset").append("svg")
      ...
      .attr("height", insetHeight + margin.top + margin.bottom + margin.bottom)
      .append("g")
      .attr("transform", "translate(" + margin.left + "," + margin.top + ")");

    // Initialize slider
    d3slider = d3.slider().min(0)
      .max(data.length - 1)
